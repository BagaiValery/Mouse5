...
public class GraficDisplay extends JPanel {
	    ...
	    private space zone = new space();
	    private Stack<space> stack = new Stack<space>();
		
		public GraficDisplay() {
		...
		Mouse mouseMotionHandler = new Mouse();
		...
		*/
		...
		for (Double[] point: graphicsData) {
			...
			boolean fl=true;
			double znach1=point[1];
			double cifraposled=znach1 % 10;
			znach1/=10;
			while (znach1!=0)
			{
				double cifrado= znach1 % 10;
				znach1/=10;
				if(cifrado<cifraposled) {
				fl=false;
				break;}
				if(fl) {
		...
		protected void paintGrid(Graphics2D canvas) {
			// Установить особое начертание для линий
			canvas.setStroke(gridStroke);
			// Линии рисуются чѐрным цветом
			canvas.setColor(Color.BLACK);
			for (int i=(int) minY; i<maxY; i++)
			canvas.draw(new Line2D.Double(xyToPoint(minX, i), xyToPoint(maxX, i)));
			for (int j=(int) minX; j<maxX; j++)
				canvas.draw(new Line2D.Double(xyToPoint(j, minY), xyToPoint(j, maxY)));
		}
		...
		*/
		...
		public class Mouse extends GraficDisplay implements MouseListener, MouseMotionListener  {
			...
			public void mouseDragged(MouseEvent arg0) {
				if (selMode) {
		            if (!transform)
		                rect.setFrame(mausePX, mausePY, arg0.getX() - rect.getX(),
		                        arg0.getY() - rect.getY());
		            else
		                rect.setFrame(-mausePY + getHeight(), mausePX, -arg0.getY()
		                        + mausePY, arg0.getX() - mausePX);
		            repaint();
				}
				 if (dragMode) {
		             if (!transform) {
		                 if (pointToXY(arg0.getX(), arg0.getY()).y < maxY && pointToXY(arg0.getX(), arg0.getY()).y > minY) {
		                     graphicsData[SMP.n][1] = pointToXY(arg0.getX(), arg0.getY()).y;
		                     SMP.yd = pointToXY(arg0.getX(), arg0.getY()).y;
		                     SMP.y = arg0.getY();
		                 }
		             } else {
		                 if (pointToXY(arg0.getX(), arg0.getY()).y < maxY && pointToXY(arg0.getX(), arg0.getY()).y > minY) {
